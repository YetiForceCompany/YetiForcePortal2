...
{strip}
	...
	<div class="input-group dateTime">
		<input name="{$FIELD_MODEL->getName()}" class=" {if !$FIELD_MODEL->isEditableReadOnly()} dateTimePickerField {/if} form-control"
			value="{$FIELD_MODEL->getEditViewDisplayValue($RECORD)}"
			id="{$MODULE_NAME}_editView_fieldName_{$FIELD_MODEL->getName()}" type="text" data-hour-format="{$USER->getPreferences('hour_format')}"
			data-date-format="{$USER->getPreferences('date_format')}" type="text" tabindex="{$FIELD_MODEL->getTabIndex()}"
