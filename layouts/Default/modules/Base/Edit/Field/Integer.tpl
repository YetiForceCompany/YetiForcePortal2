...
{strip}
<!-- tpl-Base-Edit-Field-Integer -->
{assign var=FIELD_INFO value=\App\Purifier::encodeHtml(\App\Json::encode($FIELD_MODEL->getFieldInfo()))}
{assign var=SPECIAL_VALIDATOR value=$FIELD_MODEL->getValidator()}
{assign var=PARAMS value=$FIELD_MODEL->getFieldParams()}
{assign var=FIELD_VALUE value=$FIELD_MODEL->getEditViewDisplayValue($RECORD)}
<input name="{$FIELD_MODEL->getName()}" id="{$MODULE_NAME}_editView_fieldName_{$FIELD_MODEL->getName()}" title="{\App\Language::translate($FIELD_MODEL->getFieldLabel(), $MODULE_NAME)}" type="text" class="form-control"
	   data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Base_Validator_Js.invokeValidation]]" tabindex="{$FIELD_MODEL->getTabIndex()}"
