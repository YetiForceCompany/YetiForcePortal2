...
{strip}
	...
	<li role="presentation" class="tpl-menu-HomeIcon c-menu__item nav-item menuHomeIcon" data-id="{$ITEM_MENU['id']}">
		<a class="nav-link{if $ACTIVE == 'true'} active {else} collapsed {/if} hasIcon js-submenu-toggler" {if $HASCHILDS == 'true'} data-toggle="collapse" data-target="#submenu-{$ITEM_MENU['id']}" role="button"{/if} href="index.php?module=Home&view=Dashboard" aria-haspopup="true" aria-expanded="{$ACTIVE}" aria-controls="submenu-{$ITEM_MENU['id']}">
