...
{strip}
	...
	<select id="{$MODULE_NAME}_{$VIEW}_fieldName_{$FIELD_MODEL->getName()}" title="{$FIELD_MODEL->getLabel()}" multiple
		...
		{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$FIELD_MODEL->getPicklistValues()}
			<option value="{\App\Functions::toSafeHTML($PICKLIST_NAME)}" {if in_array(\App\Functions::toSafeHTML($PICKLIST_NAME), $FIELD_MODEL->getFieldValuesList())} selected {/if}{if array_key_exists($PICKLIST_NAME, $FIELD_MODEL->getNotDisplayValuesList())} class="hide" {/if}>{$PICKLIST_VALUE}</option>
		{/foreach}
