...
{strip}
	...
	<input id="{$MODULE_NAME}_editView_fieldName_{$FIELD_MODEL->get('name')}" type="text" title="{$FIELD_MODEL->get('label')}" class="validate[custom[url]] form-control" name="{$FIELD_NAME}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required{/if}]"
