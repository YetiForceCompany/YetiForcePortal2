...
{strip}
	...
	<input name="{$FIELD_NAME}" type="hidden" value="{$RECORD->getRawValue($FIELD_NAME)}"
		   title="{\YF\Core\Functions::toSafeHTML($RECORD->getDisplayValue($FIELD_NAME))}" class="sourceField"
		   data-fieldtype="{$FIELD_MODEL->get('type')}" data-field-label="{$FIELD_MODEL->get('label')}"
		   data-displayvalue="{\YF\Core\Functions::toSafeHTML($RECORD->getDisplayValue($FIELD_NAME))}"
	...
	<div class="input-group referenceGroup">
		{if $REFERENCE_LIST_COUNT > 1}
			<div class="input-group-addon noSpaces referenceModulesListGroup">
				<select id="{$MODULE_NAME}_editView_fieldName_{$FIELD_NAME}_dropDown"
						...
						title="{\YF\Core\Functions::translate('LBL_RELATED_MODULE_TYPE')}" required="required">
		...
		<input id="{$FIELD_NAME}_display" name="{$FIELD_NAME}_display" type="text"
			   title="{\YF\Core\Functions::toSafeHTML($RECORD->getDisplayValue($FIELD_NAME))}"
			   class="marginLeftZero form-control autoComplete" {if !empty($DISPLAYID)}readonly="true"{/if}
			   value="{\YF\Core\Functions::toSafeHTML($RECORD->getDisplayValue($FIELD_NAME))}"
		...
		<span class="input-group-btn cursorPointer">
			<button class="btn btn-default clearReferenceSelection" type="button"
				...
				<span id="{$MODULE_NAME}_editView_fieldName_{$FIELD_NAME}_clear" class="glyphicon glyphicon-remove-sign"
					  title="{\YF\Core\Functions::translate('LBL_CLEAR', $MODULE_NAME)}"></span>
			</button>
			<button class="btn btn-default relatedPopup" type="button"
				...
				<span id="{$MODULE_NAME}_editView_fieldName_{$FIELD_NAME}_select" class="glyphicon glyphicon-search"
					  title="{\YF\Core\Functions::translate('LBL_SELECT', $MODULE_NAME)}"></span>
			</button>
		</span>
	</div>
{/strip}
