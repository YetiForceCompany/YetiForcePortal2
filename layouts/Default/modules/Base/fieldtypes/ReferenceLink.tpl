...
{strip}
	...
	{if $REFERENCE_LIST_COUNT gt 1}
		...
		{/if}
	{/if}
	<input name="{$FIELD_NAME}" type="hidden" value="{$RECORD->getRawValue($FIELD_NAME)}" title="{\YF\Core\Functions::toSafeHTML($RECORD->getEditViewDisplayValue($FIELD_NAME))}" class="sourceField" data-fieldtype="{$FIELD_MODEL->get('type')}" data-field-label="{$FIELD_MODEL->get('label')}" data-displayvalue="{\YF\Core\Functions::toSafeHTML($RECORD->getEditViewDisplayValue($FIELD_NAME))}" data-fieldinfo='{$FIELD_INFO}' {if $FIELD_MODEL->isEditableReadOnly()}readonly="readonly"{/if}>
	<div class="input-group referenceGroup">
		...
		{/if}
		<input id="{$FIELD_NAME}_display" name="{$FIELD_NAME}_display" type="text" title="{\YF\Core\Functions::toSafeHTML($RECORD->getEditViewDisplayValue($FIELD_NAME))}" class="marginLeftZero form-control autoComplete" {if !empty($DISPLAYID)}readonly="true"{/if}
			   value="{\YF\Core\Functions::toSafeHTML($RECORD->getEditViewDisplayValue($FIELD_NAME))}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required{/if}]"
