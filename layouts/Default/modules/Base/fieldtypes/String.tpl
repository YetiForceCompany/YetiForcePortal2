...
{strip}
	...
	<input id="{$MODULE_NAME}_editView_fieldName_{$FIELD_NAME}" type="text" title="{$FIELD_MODEL->get('label')}" class="form-control" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true}required{/if}]" name="{$FIELD_NAME}" value="{\YF\Core\Functions::toSafeHTML($FIELD_MODEL->getRawValue())}"
