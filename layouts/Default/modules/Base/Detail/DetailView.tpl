							...
							<div class="c-card__row form-row border-bottom ">
								{foreach item=FIELD from=$FIELDS_FORM[$BLOCK['id']]}
									...
									<div class="c-card__row form-row border-bottom">
										{assign var=COUNTER value=0}
									{/if}
									{assign var=DISPLAY_VALUE value=$RECORD->getDisplayValue($FIELD->getName())}
									<div class="col-sm border-left {if $FIELD->getUIType() eq '300'} col-lg-12 border-bottom {/if}" data-name="{$FIELD->getName()}" data-type="{$FIELD->get('type')}" data-uitype="{$FIELD->getUIType()}">
										<div class="form-row align-items-start">
											<div class="fieldLabel c-card__label text-lg-right text-md-left py-2 px-1{if empty($DISPLAY_VALUE)} border-right{/if}
								 {if $FIELD->getUIType() eq '300'} col-lg-3 {else} col-lg-6 {/if}">
												<label class="col-form-label font-weight-bold p-0">
													{if $FIELD->isMandatory() eq true}<span class="redColor">*</span>{/if}
													{$FIELD->get('label')}
												</label>
											</div>
											<div class="fieldValue justify-content-between c-card__value{if !empty($DISPLAY_VALUE)} border-left{/if}
								 {if $FIELD->getUIType() eq '300'}col-lg-9{else}col-lg-6{/if} py-2 px-1">
												{$DISPLAY_VALUE}
											</div>
