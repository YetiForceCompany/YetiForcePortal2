...
/* {[The file is published on the basis of YetiForce Public License 4.0 that can be found in the following directory: licenses/LicenseEN.txt or yetiforce.com]} */jQuery.Class("Base_Helper_Js",{checkServerConfigResponseCache:"",langCode:"",/*
		 ...
		 */getEmailMassEditInstance:function getEmailMassEditInstance(){var emailMassEditInstance=new window.Emails_MassEdit_Js;return emailMassEditInstance},getDayFromDate:function getDayFromDate(date){var dayOfWeek=this.getDay(date);return this.getLabelDayFromDate(dayOfWeek)},getDay:function getDay(date){var dateObj=new Date(date);return isNaN(dateObj.getDay())&&(dateObj=Date.parse(date)),dateObj.getDay()},getLabelDayFromDate:function getLabelDayFromDate(day){var dayOfWeek=day,dayOfWeekLabel="";return 0===dayOfWeek?dayOfWeekLabel="JS_SUN":1===dayOfWeek?dayOfWeekLabel="JS_MON":2===dayOfWeek?dayOfWeekLabel="JS_TUE":3===dayOfWeek?dayOfWeekLabel="JS_WED":4===dayOfWeek?dayOfWeekLabel="JS_THU":5===dayOfWeek?dayOfWeekLabel="JS_FRI":6===dayOfWeek?dayOfWeekLabel="JS_SAT":void 0,app.translate(dayOfWeekLabel)},/*
		 * Function to get Date Instance
		 * @params date---this is the field value
		 * @params dateFormat---user date format
		 * @return date object
		 */getDateInstance:function getDateInstance(dateTime,dateFormat){var dateTimeComponents=dateTime.split(" "),dateComponent=dateTimeComponents[0],timeComponent=dateTimeComponents[1],seconds="00",dotMode="-";-1!==dateFormat.indexOf("-")&&(dotMode="-"),-1!==dateFormat.indexOf(".")&&(dotMode="."),-1!==dateFormat.indexOf("/")&&(dotMode="/");var splittedDate=dateComponent.split(dotMode),splittedDateFormat=dateFormat.split(dotMode),year=splittedDate[splittedDateFormat.indexOf("yyyy")],month=splittedDate[splittedDateFormat.indexOf("mm")],date=splittedDate[splittedDateFormat.indexOf("dd")],dateInstance=Date.parse(year+dotMode+month+dotMode+date);if(4<year.length||2<month.length||2<date.length||null===dateInstance){var errorMsg=app.translate("JS_INVALID_DATE");throw errorMsg}//Before creating date object time is set to 00
//because as while calculating date object it depends system timezone
"undefined"==typeof timeComponent&&(timeComponent="00:00:00");var timeSections=timeComponent.split(":");"undefined"!=typeof timeSections[2]&&(seconds=timeSections[2]),"undefined"!=typeof dateTimeComponents[2]&&(timeComponent+=" "+dateTimeComponents[2],"pm"===dateTimeComponents[2].toLowerCase()&&"12"!==timeSections[0]&&(timeSections[0]=parseInt(timeSections[0],10)+12),"am"===dateTimeComponents[2].toLowerCase()&&"12"===timeSections[0]&&(timeSections[0]="00")),--month;var dateInstance=new Date(year,month,date,timeSections[0],timeSections[1],seconds);return dateInstance},/*
		 * Function to show the confirmation messagebox
		 */showConfirmationBox:function showConfirmationBox(data){var aDeferred=jQuery.Deferred();bootbox.setLocale(this.getLangCode());var bootBoxModal=bootbox.confirm(data.message,function(result){result?aDeferred.resolve():aDeferred.reject();});return bootBoxModal.on("hidden",function(){0<jQuery("#globalmodal").length&&jQuery("body").addClass("modal-open");}),aDeferred.promise()},showMessage:function showMessage(params){"undefined"==typeof params.type&&(params.type="info"),params.animation="show",params.title=app.translate("JS_MESSAGE"),Base_Helper_Js.showPnotify(params);},/*
		 * Function to show pnotify message
		 */showPnotify:function showPnotify(customParams){var userParams=customParams;"string"==typeof customParams&&(userParams={},userParams.text=customParams);var params={target:document.body,data:{type:"error",hide:!1,delay:"2000",modules:{Buttons:{closerHover:!1,labels:{close:app.translate("JS_CLOSE")}},Animate:{animate:!1,inClass:"zoomInLeft",outClass:"zoomOutRight"}}}};return "undefined"!=typeof customParams.type&&"error"!=customParams.type&&(params.data.hide=!0),params.data=$.extend(params.data,userParams),new PNotify(params)},/*
		 * Function to remove pnotify message
		 */hidePnotify:function hidePnotify(notice){"undefined"==typeof notice&&(notice=jQuery(".ui-pnotify")),notice.remove();},/*
		 ...
		 */showHorizontalTopScrollBar:function showHorizontalTopScrollBar(){var container=jQuery(".contentsDiv"),topScroll=jQuery(".contents-topscroll",container),bottomScroll=jQuery(".contents-bottomscroll",container);jQuery(".bottomscroll-div",container).attr("style",""),jQuery(".topscroll-div",container).css("width",jQuery(".bottomscroll-div",container).outerWidth()),jQuery(".bottomscroll-div",container).css("width",jQuery(".topscroll-div",container).outerWidth()),topScroll.scroll(function(){bottomScroll.scrollLeft(topScroll.scrollLeft());}),bottomScroll.scroll(function(){topScroll.scrollLeft(bottomScroll.scrollLeft());});},convertToDateString:function convertToDateString(stringDate,dateFormat,modDay,type){var dotMode="-";-1!==dateFormat.indexOf("-")&&(dotMode="-"),-1!==dateFormat.indexOf(".")&&(dotMode="."),-1!==dateFormat.indexOf("/")&&(dotMode="/");var splittedDate=stringDate.split(dotMode),splittedDateFormat=dateFormat.split(dotMode),year=splittedDate[splittedDateFormat.indexOf("yyyy")],month=splittedDate[splittedDateFormat.indexOf("mm")],date=splittedDate[splittedDateFormat.indexOf("dd")],dateInstance=new Date(year,month-1,date);if(4<year.length||2<month.length||2<date.length||null===dateInstance){var errorMsg=app.translate("JS_INVALID_DATE");throw errorMsg}var newDate=dateInstance;return "0"===modDay?"Calendar"===type&&newDate.setDate(dateInstance.getDate()-1):"-1"===modDay?"Calendar"===type?newDate.setDate(dateInstance.getDate()-2):newDate.setDate(dateInstance.getDate()-1):"Calendar"!==type&&newDate.setTime(dateInstance.getTime()+1e3*(60*(60*(24*parseInt(modDay))))),app.getStringDate(newDate)},hideOptions:function hideOptions(element,attr,value){var opval="";element.find("option").each(function(index,option){option=$(option),value===option.data(attr)?(""===opval&&(opval=option.val()),option.removeClass("d-none"),option.removeAttr("disabled")):(option.addClass("d-none"),option.attr("disabled","disabled"));}),element.val(opval),element.trigger("chosen:updated");},unique:function unique(array){return array.filter(function(el,index,arr){return index===arr.indexOf(el)})}},{});
