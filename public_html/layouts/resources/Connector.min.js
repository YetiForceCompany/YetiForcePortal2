...
if("undefined"!=typeof params.url&&-1!==params.url.indexOf("?")){fullUrl=params.url;var urlSplit=params.url.split("?"),queryString=urlSplit[1];params.url=urlSplit[0];for(var queryParameters=queryString.split("&"),index=0;index<queryParameters.length;index++){var queryParam=queryParameters[index],queryParamComponents=queryParam.split("=");params.data[queryParamComponents[0]]=queryParamComponents[1];}}if(("undefined"==typeof params.url||0>=params.url.length)&&(params.url="index.php"),params.success=function(data){null!==data&&"object"===_typeof(data)&&data.error&&(app.errorLog(data.error),data.error.message&&app.showNotify({text:data.error.message,type:"error"})),aDeferred.resolve(data);},params.error=function(jqXHR,textStatus,errorThrown){"-".repeat(150);console.warn("%cYetiForce debug mode!!!","color: red; font-family: sans-serif; font-size: 1.5em; font-weight: bolder; text-shadow: #000 1px 1px;");var log="Error: "+errorThrown+"\n";if(log+="------------------------------------------------------------------------------------------------------------------------------------------------------\nParams:\n------------------------------------------------------------------------------------------------------------------------------------------------------\n"+JSON.stringify(params,null,"\t")+"\n",jqXHR.responseJSON)log+="------------------------------------------------------------------------------------------------------------------------------------------------------\nMessage: "+jqXHR.responseJSON.error.message+"\n",log+="------------------------------------------------------------------------------------------------------------------------------------------------------\nTrace:\n------------------------------------------------------------------------------------------------------------------------------------------------------\n"+jqXHR.responseJSON.error.trace+"\n";else try{var json=JSON.parse(jqXHR.responseText);log+=json.error.message?"------------------------------------------------------------------------------------------------------------------------------------------------------\nMessage: "+json.error.message+"\n":"",log+=json.error.trace?"------------------------------------------------------------------------------------------------------------------------------------------------------\nTrace:\n------------------------------------------------------------------------------------------------------------------------------------------------------\n"+json.error.trace+"\n":"";}catch(e){log+="------------------------------------------------------------------------------------------------------------------------------------------------------\nResponse:\n------------------------------------------------------------------------------------------------------------------------------------------------------\n"+jqXHR.responseText;}console.error(log),aDeferred.reject(textStatus,errorThrown,jqXHR);},jQuery.ajax(params),pjaxMode&&(""===fullUrl?fullUrl="index.php?"+$.param(params.data):-1===fullUrl.indexOf("index.php?")&&(fullUrl="index.php?"+fullUrl),history.pushState&&""!==fullUrl)){var currentHref=window.location.href;history.state||history.replaceState(currentHref,"title 1",currentHref),history.pushState(fullUrl,"title 2",fullUrl);}return aDeferred.promise()},requestForm:function requestForm(url,params){var newEle="<form action="+url+" method=\"POST\">";"undefined"!=typeof csrfMagicName&&(newEle+="<input type=\"hidden\" name=\""+csrfMagicName+"\"  value='"+csrfMagicToken+"'>"),"undefined"!=typeof params&&jQuery.each(params,function(index,value){newEle+="<input type=\"hidden\" name=\""+index+"\"  value='"+value+"'>";}),newEle+="</form>";var form=new jQuery(newEle);form.appendTo("body").submit();}};
