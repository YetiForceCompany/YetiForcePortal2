...
/*if(strippedValue < 0){
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var response=this._super();if(!1==response){var fieldValue=this.getFieldValue();if(''!==fieldValue){var doubleRegex=/(^[-+]?\d+)[,.]\d+$/;if(!fieldValue.match(doubleRegex)){var errorInfo=app.translate('JS_PLEASE_ENTER_DECIMAL_VALUE');return this.setError(errorInfo),!1}}return !0}return response}}),Vtiger_Base_Validator_Js('Vtiger_Date_Validator_Js',{/**
		 *Function which invokes field validation
		 *@param accepts field element as parameter
		 * @return error if validation fails true on success
