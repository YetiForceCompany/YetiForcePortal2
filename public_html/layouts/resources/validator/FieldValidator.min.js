...
Vtiger_Base_Validator_Js('Vtiger_Email_Validator_Js',{/**
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var response=this._super();return !0==response?!(100<App.Fields.Double.formatToDb(this.getFieldValue()))||(this.setError(app.translate('JS_PERCENTAGE_VALUE_SHOULD_BE_LESS_THAN_100')),!1):response}}),Vtiger_Base_Validator_Js('Vtiger_Url_Validator_Js',{/**
		 * Function which invokes field validation
		 * @param accepts field element as parameter
		 * @return error if validation fails true on success
		 */invokeValidation:function invokeValidation(field){var validatorInstance=new Vtiger_Url_Validator_Js;validatorInstance.setElement(field);var result=validatorInstance.validate();return !0===result?result:validatorInstance.getError()}},{/**
		 * Function to validate the Url
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var fieldValue=this.getFieldValue(),regexp=/(^|\s)((https?:\/\/)?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)/gi,result=regexp.test(fieldValue);if(result||0!==fieldValue.indexOf('http://')&&0!==fieldValue.indexOf('https://')&&0!==fieldValue.indexOf('ftp://')&&0!==fieldValue.indexOf('ftps://')&&0!==fieldValue.indexOf('telnet://')&&0!==fieldValue.indexOf('smb://')&&0!==fieldValue.indexOf('www.')||(result=!0),!result){var _errorInfo=app.translate('JS_CONTAINS_ILLEGAL_CHARACTERS');//"Please enter valid url";
...
if(0==dependentFieldInContext.val().length)continue;var dependentFieldDateInstance=this.getDateTimeInstance(dependentFieldInContext);if(0<fieldDateInstance-dependentFieldDateInstance){var errorInfo=fieldLabel+' '+app.translate('JS_SHOULD_BE_LESS_THAN_OR_EQUAL_TO')+' '+dependentFieldLabel+'';return this.setError(errorInfo),!1}}}return !0}},getDateTimeInstance:function getDateTimeInstance(field){var dateFormat=field.data('dateFormat'),fieldValue=field.val();try{var dateTimeInstance=Vtiger_Helper_Js.getDateInstance(fieldValue,dateFormat);}catch(err){return this.setError(err),!1}return dateTimeInstance}}),Vtiger_Base_Validator_Js('Vtiger_Currency_Validator_Js',{/**
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var response=this._super();if(!0!=response)return response;var fieldData=this.getElement().data(),decimalSeparator=fieldData.decimalSeparator?fieldData.decimalSeparator:CONFIG.currencyDecimalSeparator,groupSeparator=fieldData.groupSeparator?fieldData.groupSeparator:CONFIG.currencyGroupingSeparator,strippedValue=this.getFieldValue().replace(decimalSeparator,''),spacePattern=/\s/;(spacePattern.test(decimalSeparator)||spacePattern.test(groupSeparator))&&(strippedValue=strippedValue.replace(/ /g,''));var errorInfo;console.log(spacePattern),'$'===groupSeparator&&(groupSeparator='\\$'),'.'===groupSeparator&&(groupSeparator='\\.');var regex=new RegExp(groupSeparator,'g');if(strippedValue=strippedValue.replace(regex,''),isNaN(strippedValue))return errorInfo=app.translate('JS_CONTAINS_ILLEGAL_CHARACTERS'),this.setError(errorInfo),!1;var negativeNumber=fieldData.fieldinfo&&'NN'===fieldData.fieldinfo.fieldtype;if(!negativeNumber&&0>strippedValue)return errorInfo=app.translate('JS_ACCEPT_POSITIVE_NUMBER'),this.setError(errorInfo),!1;var maximumLength='undefined'==typeof fieldData.fieldinfo?null:fieldData.fieldinfo.maximumlength;return !(maximumLength&&strippedValue>parseFloat(maximumLength))||(errorInfo=app.translate('JS_ERROR_MAX_VALUE'),this.setError(errorInfo),!1)}}),Vtiger_Base_Validator_Js('Vtiger_NumberUserFormat_Validator_Js',{/**
		 *Function which invokes field validation
		 *@param accepts field element as parameter
		 * @return error if validation fails true on success
		 ...
		 *@param accepts field element as parameter
		 * @return error if validation fails true on success
		 */invokeValidation:function invokeValidation(field){var doubleValidator=new Vtiger_Double_Validator_Js;doubleValidator.setElement(field);var response=doubleValidator.validate();if(!0!=response)return doubleValidator.getError()}},{/**
		 * Function to validate the Decimal field data
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var response=this._super();if(!0===response){var fieldValue=this.getFieldValue(),doubleRegex=/(^[-+]?\d+)(\.\d+)?$/;if(!fieldValue.toString().match(doubleRegex)){var _errorInfo2=app.translate('JS_PLEASE_ENTER_DECIMAL_VALUE');return this.setError(_errorInfo2),!1}var fieldInfo=this.getElement().data().fieldinfo;if(!fieldInfo||!fieldInfo.maximumlength)return !0;var maximumLength=fieldInfo.maximumlength,minimumLength=-maximumLength;fieldValue=parseFloat(fieldValue);var ranges=maximumLength.split(',');if(2===ranges.length&&(maximumLength=ranges[1],minimumLength=ranges[0]),fieldValue>parseFloat(maximumLength)||fieldValue<parseFloat(minimumLength))return errorInfo=app.translate('JS_ERROR_MAX_VALUE'),this.setError(errorInfo),!1}return response},/**
		 * Overwrites base function to avoid trimming and validate white spaces
		 * @return fieldValue
		 * */getFieldValue:function getFieldValue(){return App.Fields.Double.formatToDb(this.getElement().val())}}),Vtiger_Base_Validator_Js('Vtiger_Date_Validator_Js',{/**
		 * Function which invokes field validation
		 * @param accepts field element as parameter
		 * @return error if validation fails true on success
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var field=this.getElement(),fieldData=field.data(),fieldDateFormat=fieldData.dateFormat,fieldValue=this.getFieldValue();console.log(field,fieldData,fieldDateFormat,fieldValue);try{if('range'!==fieldData.calendarType)fieldValue=[fieldValue];else if(fieldValue=fieldValue.split(','),2!==fieldValue.length)throw new Error;fieldValue.forEach(function(key){Vtiger_Helper_Js.getDateInstance(key,fieldDateFormat);});}catch(err){var _errorInfo3=app.translate('JS_PLEASE_ENTER_VALID_DATE');return this.setError(_errorInfo3),!1}return !0}}),Vtiger_Base_Validator_Js('Vtiger_Time_Validator_Js',{/**
		 * Function which invokes field validation
		 * @param accepts field element as parameter
		 * @return error if validation fails true on success
