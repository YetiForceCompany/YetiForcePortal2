...
files.forEach(function(file){App.Fields.MultiImage.currentFileUploads--,_this.deleteFile(file.hash,!1),app.showNotify({text:app.translate("JS_FILE_UPLOAD_ERROR")+" [".concat(file.name,"]"),type:"error"});}),this.updateFormValues())}/**
	 ...
	 */},{key:"deleteFile",value:function deleteFile(hash){var _this4=this,showConfirmation=!(1<arguments.length&&void 0!==arguments[1])||arguments[1];if(showConfirmation){var fileInfo=this.getFileInfo(hash);app.showConfirmModal({title:fileInfo.name,text:app.translate("JS_DELETE_FILE_CONFIRMATION"),titleTrusted:!1,icon:"fa fa-trash-alt",confirmedCallback:function confirmedCallback(){_this4.deleteFileCallback(hash);}});}else this.deleteFileCallback(hash);}/**
	 * File change event handler from jQuery-file-upload
	 *
	 * @param {Event} e
	 * @param {object} data
	 */},{key:"change",value:function change(e,data){var _this5=this;this.clearFilesInput(),data.files=this.filterValidFiles(data.files),data.files=this.setFilesHash(data.files),data.files.length&&this.generatePreviewElements(data.files,function(){_this5.redraw();});}},{key:"clearFilesInput",value:function clearFilesInput(){var _this6=this;this.files.forEach(function(file){file.file&&_this6.deleteFile(file.hash,!1);});}/**
	 ...
	 * @param {Event} e
	 * @param {Object} ui
	 */},{key:"sortStop",value:function sortStop(){var _this7=this,actualElements=this.elements.result.find(".js-multi-image__preview").toArray();this.files=actualElements.map(function(element){for(var elementHash,i=0,len=_this7.files.length;i<len;i++)if(elementHash=$(element).data("hash"),_this7.files[i].hash===elementHash)return _this7.files[i]}),this.redraw();}/**
	 * Redraw view according to in-memory positions
	 */},{key:"redraw",value:function redraw(){var _this8=this;this.files.forEach(function(file){_this8.elements.result.append(file.previewElement);}),this.updateFormValues();}/**
	 * Enable drag and drop files repositioning
	 */},{key:"enableDragNDrop",value:function enableDragNDrop(){this.elements.result.sortable({handle:".js-multi-image__preview-img",items:".js-multi-image__preview",over:this.sortOver.bind(this),stop:this.sortStop.bind(this)}).disableSelection().on("mousedown",".js-multi-image__preview-img",function(){this.focus();});}},{key:"generatePreviewElements",value:function generatePreviewElements(files,callback){var _this9=this;files.forEach(function(file){if(file instanceof File){var template=$(_this9.generatePreviewFromFile(file));file.preview=template,_this9.addFileInfoProperty(file.hash,"previewElement",file.preview),callback(template);}else _this9.generatePreviewFromValue(file,function(template,imageSrc){file.preview=_this9.addPreviewPopover(file,template,imageSrc),_this9.addFileInfoProperty(file.hash,"previewElement",file.preview),callback(file.preview);});});}/**
	 ...
	 */},{key:"loadExistingFiles",value:function loadExistingFiles(){var _this10=this;this.files=this.files.map(function(file){return file.hash=_this10.generateRandomHash(CONFIG.userId),file}).slice(0,this.options.limit),this.generatePreviewElements(this.files,function(element){_this10.elements.result.append(element);}),this.updateFormValues();}/**
