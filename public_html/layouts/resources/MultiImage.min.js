...
var MultiImage=function(){/**
	 ...
	 */function MultiImage(element){classCallCheck(this,MultiImage);var thisInstance=this;this.elements={},this.options={showCarousel:!0},this.detailView=!1,this.elements.fileInput=element.find('.js-multi-image__file').eq(0),0===this.elements.fileInput.length&&(this.detailView=!0),this.elements.component=element.eq(0),this.elements.form=element.closest('form').eq(0),this.elements.addButton=this.elements.component.find('.js-multi-image__file-btn').eq(0),this.elements.values=this.elements.component.find('.js-multi-image__values').eq(0),this.elements.progressBar=this.elements.component.find('.js-multi-image__progress-bar').eq(0),this.elements.progress=this.elements.component.find('.js-multi-image__progress').eq(0),this.elements.result=this.elements.component.find('.js-multi-image__result').eq(0),this.fieldInfo=this.elements.values.data('fieldinfo'),this.options.formats=this.fieldInfo.formats,this.options.limit=this.fieldInfo.limit,this.files=this.detailView?this.elements.values.data('value'):JSON.parse(this.elements.values.val()),this.detailView||this.elements.fileInput.fileupload({dataType:'json',replaceFileInput:!1,fileInput:this.fileInput,autoUpload:!1,add:this.add.bind(this),change:this.change.bind(this),fail:this.uploadError.bind(this)}),this.elements.component.on('click','.js-multi-image__popover-img',function(){thisInstance.zoomPreview($(this).data('hash'));}),this.elements.component.on('click','.js-multi-image__popover-btn-zoom',function(e){e.preventDefault(),thisInstance.zoomPreview($(this).data('hash'));}),this.elements.component.on('click','.js-multi-image__popover-btn-download',function(e){e.preventDefault(),thisInstance.download($(this).data('hash'));}),this.detailView||this.elements.component.on('click','.js-multi-image__popover-btn-delete',function(e){e.preventDefault(),thisInstance.deleteFile($(this).data('hash'));}),this.loadExistingFiles();}/**
	 * Prevent form submission
	 *
	 * @param {Event} e
	 */return createClass(MultiImage,[{key:'addButtonClick',value:function addButtonClick(e){e.preventDefault(),this.elements.fileInput.trigger('click');}/**
	 * Get file information
	 *
	 * @param {String} hash - file id
	 * @returns {Object}
	 */},{key:'getFileInfo',value:function getFileInfo(hash){for(var file,i=0,len=this.files.length;i<len;i++)if(file=this.files[i],file.hash===hash)return file;app.errorLog('File \''+hash+'\' not found.'),app.showNotify({text:app.translate('JS_INVALID_FILE_HASH')+(' ['+hash+']'),type:'error'});}/**
...
return 'undefined'!=typeof response.result&&'undefined'!=typeof response.result.attach&&Array.isArray(response.result.attach)?(response.result.attach.forEach(function(fileAttach){App.Fields.MultiImage.currentFileUploads--,_this.deleteFile(fileAttach.hash,!1),'string'==typeof fileAttach.error?app.showNotify({text:fileAttach.error+(' ['+fileAttach.name+']'),type:'error'}):app.showNotify({text:app.translate('JS_FILE_UPLOAD_ERROR')+(' ['+fileAttach.name+']'),type:'error'});}),void this.updateFormValues()):void(// else show default upload error
files.forEach(function(file){App.Fields.MultiImage.currentFileUploads--,_this.deleteFile(file.hash,!1),app.showNotify({text:app.translate('JS_FILE_UPLOAD_ERROR')+(' ['+file.name+']'),type:'error'});}),this.updateFormValues())}/**
	 ...
	 * @param {Object} file
	 * @returns {boolean}
	 */},{key:'validateFile',value:function validateFile(file){var valid=!1;return this.options.formats.forEach(function(format){file.type==='image/'+format&&(valid=!0);}),valid||app.showNotify({text:app.translate('JS_INVALID_FILE_TYPE')+' ['+file.name+']\n'+app.translate('JS_AVAILABLE_FILE_TYPES')+'  ['+this.options.formats.join(', ')+']',type:'error'}),valid}/**
	 * Show limit error
	 */},{key:'showLimitError',value:function showLimitError(){this.elements.fileInput.val(''),app.showNotify({text:app.translate('JS_FILE_LIMIT')+' ['+this.options.limit+']',type:'error'});}/**
	 ...
	 *
	 * @param {string} hash
	 */},{key:'zoomPreview',value:function zoomPreview(hash){var _this4=this,thisInstance=this,fileInfo=this.getFileInfo(hash),titleTemplate=function(){return '<i class="fa fa-image"></i> '+fileInfo.name},bootboxOptions={size:'large',backdrop:!0,onEscape:!0,title:'<span id="bootbox-title-'+hash+'">'+titleTemplate()+'</span>',message:'<img src="'+fileInfo.imageSrc+'" class="w-100" />',buttons:{}};this.options.showCarousel&&(bootboxOptions.message=this.generateCarousel(hash)),this.detailView||(bootboxOptions.buttons.Delete={label:'<i class="fa fa-trash-alt"></i> '+app.translate('JS_DELETE'),className:'float-left btn btn-danger',callback:function callback(){thisInstance.deleteFile(fileInfo.hash);}}),bootboxOptions.buttons.Download={label:'<i class="fa fa-download" title="'+app.translate('JS_DOWNLOAD')+'"></i>',className:'float-left btn btn-success',callback:function callback(){thisInstance.download(fileInfo.hash);}},bootboxOptions.buttons.Close={label:'<i class="fa fa-times" title="'+app.translate('JS_CLOSE')+'"></i>',className:'btn btn-warning',callback:function callback(){}},bootbox.dialog(bootboxOptions),this.options.showCarousel&&$('#carousel-'+hash).on('slid.bs.carousel',function(e){fileInfo=_this4.getFileInfo($(e.relatedTarget).data('hash')),$('#bootbox-title-'+hash).html(titleTemplate());});}/**
	 ...
	 */},{key:'deleteFile',value:function deleteFile(hash){var _this5=this,showConfirmation=!(1<arguments.length&&void 0!==arguments[1])||arguments[1];if(showConfirmation){var fileInfo=this.getFileInfo(hash);bootbox.confirm({title:'<i class="fa fa-trash-alt"></i> '+app.translate('JS_DELETE_FILE'),message:app.translate('JS_DELETE_FILE_CONFIRMATION')+' <span class="font-weight-bold">'+fileInfo.name+'</span>',callback:function callback(result){result&&_this5.deleteFileCallback(hash);}});}else this.deleteFileCallback(hash);}/**
	 ...
	 */},{key:'generatePreviewFromFile',value:function generatePreviewFromFile(file){return '<div class="mr-1 js-multi-image__preview d-none" id="js-multi-image__preview-hash-'+file.hash+'" data-hash="'+file.hash+'" data-js="container|click">\n\t\t\t<button class="btn btn-sm btn-danger c-btn-collapsible js-multi-image__popover-btn-delete mr-1" type="button" data-hash="'+file.hash+'" data-js="click" title="'+app.translate('JS_DELETE')+'"><i class="fa fa-trash-alt"></i></button>'+file.name+'\n\t\t</div>'}/**
	 * Generate preview of image as html string from existing values
	 *
	 * @param {File} file
	 * @param {function} callback
	 */},{key:'generatePreviewFromValue',value:function generatePreviewFromValue(file,callback){callback('<div class="d-inline-block mr-1 js-multi-image__preview" id="js-multi-image__preview-hash-'+file.hash+'" data-hash="'+file.hash+'" data-js="container|click">\n\t\t\t\t<div class="img-thumbnail js-multi-image__preview-img c-multi-image__preview-img" data-hash="'+file.hash+'" data-js="drag" style="background-image:url('+file.imageSrc+')" tabindex="0" title="'+file.name+'"></div>\n\t\t</div>',file.imageSrc);}/**
