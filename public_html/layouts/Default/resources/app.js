...
var AppConnector,
	app = {
		...
		getPageController: function() {
			...
			if (
				typeof window[moduleClassName] == "undefined" &&
				extendModules != undefined
			) {
		...
		parseNumberToFloat: function(val) {
			...
			var decimalSeparator = app.getMainParams(
		...
		registerTimeField: function(container) {
			...
			timeFieldElements.each(function() {
				...
				var fieldInfo = app.parseFieldInfo(
					input.attr("data-fieldinfo")
				);
		...
		registerDateField: function(container) {
			...
			dateFieldElements.each(function() {
				...
				if (fieldInfo["day-of-the-week-int"]) {
					params_custom.locale.firstDay =
						fieldInfo["day-of-the-week-int"];
				}
				...
				input.on("apply.daterangepicker", function(ev, picker) {
					$(this).val(
						picker.startDate.format(params_custom.locale.format)
					);
				});
			});
		},
		registerChznSelectField: function(parent, view, params) {
			...
			if (selectElement.filter("[multiple]")) {
				params.placeholder_text_multiple =
					" " + app.translate("JS_SELECT_SOME_OPTIONS");
			}
			params.placeholder_text_single =
				" " + app.translate("JS_SELECT_AN_OPTION");
			...
			var chosenSelectConainer = jQuery(
		...
		showModalWindow: function(data, url, cb, paramsObject) {
			...
			var showModalData = function(data) {
				...
				thisInstance.registerDataTables(
					modalContainer.find(".dataTable")
				);
		...
		registerSubMenu: function() {
			$(".js-submenu-toggler").on("click", e => {
				if (
					!$(e.currentTarget).hasClass("collapsed") &&
