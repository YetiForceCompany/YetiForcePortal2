...
window.Base_EditView_Js=function(){/**
	 * Constructor.
	 */function _class(){classCallCheck(this,_class),this.container=void 0,this.form=void 0,this.moduleName=void 0;}/**
	 * Get container.
	 */return createClass(_class,null,[{key:'getInstanceByModuleName',/**
	 * Function to get Instance by name
	 *
	 * @param {string} moduleName Name of the module to create instance
	 *
	 * @returns {Base_EditView_Js}
	 */value:function getInstanceByModuleName(moduleName){'undefined'==typeof moduleName&&(moduleName=app.getModuleName());var instance,modalClass=moduleName+'_EditView_Js';return 'undefined'==typeof window[modalClass]&&(modalClass='Base_EditView_Js'),'undefined'!=typeof window[modalClass]&&(instance=new window[modalClass]),instance.moduleName=moduleName,instance}}]),createClass(_class,[{key:'getContainer',value:function getContainer(){return void 0===this.container&&(this.container=jQuery('.mainContent'),this.form=this.container.find('form'),this.moduleName=this.form.find('[name="module"]').val()),this.container}/**
	 ...
	 */},{key:'clearFieldValue',value:function clearFieldValue(element){var fieldValueContender=element.closest('.fieldValue'),fieldNameElement=fieldValueContender.find('.sourceField'),fieldName=fieldNameElement.attr('name');fieldNameElement.val(''),fieldValueContender.find('input[data-display="'+fieldName+'"]').val('');}/**
	 * Set reference field value.
	 *
	 * @param {jQuery} fieldContainer
	 * @param {object} params
	 */},{key:'setReferenceFieldValue',value:function setReferenceFieldValue(fieldContainer,params){var sourceField=fieldContainer.find('input.sourceField').attr('name');fieldContainer.find('input[name="'+sourceField+'"]').val(params.id),fieldContainer.find('input[data-display="'+sourceField+'"]').val(params.name);}/**
	 ...
	 * Register tree.
	 */},{key:'registerTree',value:function registerTree(){this.form.find('.js-tree-select').on('click',function(e){var containerField=$(e.currentTarget).closest('.js-tree-content'),treeValueField=containerField.find('.js-tree-value'),fieldDisplayElement=containerField.find('input[data-display="'+treeValueField.attr('name')+'"]'),multiple=treeValueField.data('multiple');app.showModalWindow(containerField.find('.js-tree-modal-window').clone(),function(modalContainer){var jstreeInstance=modalContainer.find('.js-tree-jstree');1===multiple?new window.App.Fields.MultiTree(jstreeInstance):new window.App.Fields.Tree(jstreeInstance),modalContainer.find('.js-tree-modal-select').on('click',function(){var selectedCategories=[];if($.each(jstreeInstance.jstree('get_selected',!0),function(index,value){selectedCategories.push(value);}),0!=selectedCategories.length){var treeText=[],treeValue=[];$.each(selectedCategories,function(index,value){treeValue.push(value.original.tree),treeText.push(value.text);}),fieldDisplayElement.val(treeText.join(',')),fieldDisplayElement.attr('readonly',!0),treeValueField.val(treeValue.join(',')),app.hideModalWindow('',modalContainer.data('modalId')),PNotify.defaultStack.close();}else app.showNotify({text:app.translate('JS_PLEASE_SELECT_ATLEAST_ONE_OPTION'),type:'error'});});});}),this.form.find('.js-tree-clear').on('click',function(e){var containerField=$(e.currentTarget).closest('.js-tree-content'),treeValueField=containerField.find('.js-tree-value'),fieldDisplayElement=containerField.find('input[data-display="'+treeValueField.attr('name')+'"]');fieldDisplayElement.val(''),fieldDisplayElement.attr('readonly',!1),treeValueField.val('');});}/**
	 * Save record
	 */},{key:'save',value:function save(params){if(!this.form.validationEngine('validate'))return void app.formAlignmentAfterValidation(this.form);var callbackBeforeSave=params.callbackBeforeSave||function(){},callbackAfterSave=params.callbackAfterSave||function(){},progress=$.progressIndicator({message:app.translate('JS_SAVE_LOADER_INFO'),position:'html',blockInfo:{enabled:!0}}),beforeSaveResult=callbackBeforeSave(this.form.serializeFormData());if(!1===beforeSaveResult)return void progress.progressIndicator({mode:'hide'});var formData=new FormData(this.form[0]);AppConnector.request({url:'index.php',type:'POST',data:formData,processData:!1,contentType:!1}).done(function(response){progress.progressIndicator({mode:'hide'}),response.success&&app.showNotify({text:app.translate('JS_SAVE_NOTIFY_SUCCESS'),type:'success'}),callbackAfterSave(response.result);}).fail(function(textStatus,errorThrown){callbackAfterSave(!1),app.showNotify({text:errorThrown,title:app.translate('JS_ERROR'),type:'error'});});}/**
	 ...
	 */},{key:'registerFormEvents',value:function registerFormEvents(){this.getContainer(),this.registerReferenceEvent(),this.registerFieldsValidations(),this.registerTree(),this.form.find(':input').inputmask();}/**
	 * Register edit view events.
	 */},{key:'registerEvents',value:function registerEvents(){this.getContainer(),this.registerFormEvents(),this.registerEditViewEvents();}}]),_class}();
